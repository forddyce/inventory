...
@section('title')
Pelunasan Hutang #{{ $model->id }} - {{ config('app.name') }}
@stop
...
<div id="page-container" class="sidebar-o side-scroll page-header-modern main-content-narrow">
  ...
  <main id="main-container">
    <div class="content">
      <h2 class="content-heading">Data Pelunasan Hutang</h2>
      <div class="row">
        <div class="col-md-6">
          <div class="block">
            <div class="block-header block-header-default">
              <h3>Pelunasan Hutang #{{ $model->id }}</h3>
            </div>
            ...
            <div class="block-content block-content-full">
              <div class="table-responsive">
                <table class="table table-striped table-bordered">
                  ...
                  <?php $invoices = json_decode($model->debt_infos, 1); $total = 0; ?>
                  <tbody>
                    @if (count($invoices) > 0)
                      @foreach ($invoices as $invoice)
                      <?php
                        $link = '#';
                        if ($debt = $debtRepo->findById($invoice['debt_id'])) {
                          if ($purchase = $debt->purchase) {
                            $link = route('purchase.invoice.print', ['id' => $purchase->id]);
                          }
                        }
                      ?>
                      <tr>
