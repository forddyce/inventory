...
@section('title')
Buat Penjualan - {{ config('app.name') }}
@stop
...
@section('content')
  <div id="page-container" class="sidebar-o side-scroll page-header-modern main-content-narrow">
    ...
    <main id="main-container">
      <div class="content">
        <h2 class="content-heading">Tambah Data Penjualan</h2>
        <div class="row">
          <div class="col-md-6">
            <div class="block">
              ...
              <div class="block-content">
                <form id="salesForm" role="form" onsubmit="return false;" data-url="{{ route('sales.add') }}">
                        ...
                        <select class="form-control js-select2" style="width: 100%;" data-placeholder="Choose one.." id="itemList">
                          @if (count($items) > 0)
                            @foreach ($items as $item)
                              <option data-id="{{ $item->id }}" data-name="{{ $item->item_name }}" data-code="{{ $item->item_code }}" data-price="{{ (float) $item->original_price }}" data-last="{{ (float) $item->last_sales_price }}">{{ $item->item_name }} (Stok: {{ $item->stock . ' ' . $item->item_unit }})</option>
                            @endforeach
                          @endif
                        </select>
...
{{--                         <span class="help-block">
                    ...
                    <div class="col-md-9">
                      <div class="form-material">
                        <label class="css-control css-control-primary css-switch">
                          ...
                          <span class="css-control-indicator"></span> Pakai harga jual terakhir?
                        </label>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="form-group row receivable-container">
                    <div class="col-md-9">
                      <div class="form-material floating">
                        <input type="number" name="receivable_amount" class="form-control" value="0" min="0">
                        <label for="inputDate">Jumlah Piutang</label>
                      </div>
                    </div>
                  </div>
          ...
          <div class="col-md-6">
            <div class="block">
              <div class="block-header block-header-default">
                <h3 class="block-title">Data Penjualan</h3>
              </div>
              <div class="block-content">
